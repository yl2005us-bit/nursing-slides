...
import { ChevronLeft, ChevronRight, MessageCircle, Sprout, Hammer, Zap, CheckCircle2, XCircle, Timer, Megaphone, Milestone, ArrowRight, UserPlus, Home } from 'lucide-react';
    ...
    <div className={`fixed inset-0 ${slide.bg} transition-colors duration-700 flex flex-col font-sans overflow-y-auto text-slate-900 z-50`}>
      ...
      <header className="px-8 py-6 flex justify-between items-center z-10 mt-2">
        <div className="flex items-center space-x-3">
          ...
          <span className={`text-sm font-bold tracking-widest uppercase ${isDarkBg ? 'text-slate-400' : 'text-slate-500'}`}>Part 3: Seedlings of Change</span>
        </div>
        ...
        <div className="w-full max-w-6xl animate-in fade-in slide-in-from-bottom-4 duration-700 flex flex-col items-center">
          {slide.type === "cover" ? (
            <div className="text-center text-white space-y-8 py-12">
              ...
              <div className="space-y-4">
                <p className="text-2xl md:text-4xl font-light text-emerald-200 tracking-widest">
                  {slide.subtitle}
                </p>
              </div>
              <div className="h-1.5 w-32 bg-emerald-500 mx-auto mt-12"></div>
            </div>
          ) : (
            <>
      ...
      <footer className="px-8 py-8 flex justify-between items-end relative z-40">
        ...
        <div className="relative">
          <button onClick={() => setShowNotes(!showNotes)} className={`flex items-center space-x-2 px-6 py-4 rounded-full font-bold transition-all shadow-lg ${showNotes ? 'bg-emerald-600 text-white' : 'bg-white text-slate-600 hover:bg-slate-50 border border-slate-200'}`}>
